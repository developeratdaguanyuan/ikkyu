...
class BiGRUCRF(object):
  def __init__(self, vocab_size, class_size, word_vectors,
    ...
    # Project Layer
    self.logits = self._apply_tanh(z, self.hidden_dim * 2, self.class_sz, 'logit_project')
  ...
  def _apply_tanh(self, inputs, input_unit_num, output_unit_num, scope=None, reuse=False):
    rank = len(inputs.get_shape())
    if rank == 3:
      time_steps = tf.shape(inputs)[1]
      inputs_2d = tf.reshape(inputs, [-1, input_unit_num])
    ...
    scope_name = scope or 'tanh'
    with tf.variable_scope(scope_name, reuse=reuse):
      initializer = tf.random_uniform_initializer(-0.1, 0.1)
      W = tf.get_variable('W', [input_unit_num, output_unit_num], initializer=initializer)
      b = tf.get_variable('b', [output_unit_num], initializer=initializer)
      z = tf.tanh(tf.matmul(inputs_2d, W) + b)
    if rank == 3:
      output_shape = tf.stack([-1, time_steps, output_unit_num])
      return tf.reshape(z, output_shape)
  ...
  def train(self, train_data_producer, valid_data_producer, num_epoch):
    logging.basicConfig(filename='../logs/bigru_crf.log', filemode='w', level=logging.INFO)
    saver = tf.train.Saver()
    with tf.Session() as self.sess:
      self.sess.run(tf.global_variables_initializer())

      train_loss, min_valid_loss, min_valid_error = 0, float('inf'), 0
      ...
      for i in range(num_iteration):
        ...
        feed = {self.words: words, self.tags: tags, self.lengths: lengths}
        ...
        if i > 0 and i % (train_data_producer.size / self.batch) == 0:
          ...
          if valid_loss <= min_valid_loss:
            ...
            save_path = saver.save(self.sess, "../save_models/bigru_crf_" + str(i))
