...
train_path = "../../data/subject/subject_train.txt"
valid_path = '../../data/subject/subject_valid.txt'
...
def main(_):
    ...
    graph = bigru.BiGRU(len(word_embedding), word_embedding)
    graph.train(train_data_producer, valid_data_producer, 100)

    # graph = gru.GRU(len(word_embedding), word_embedding)
    # graph.train(train_data_producer, valid_data_producer, 100)
