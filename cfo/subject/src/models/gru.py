...
class GRU(object):
  def __init__(self, vocab_size, word_vectors,
    ...
    tags_p = self._apply_relu(self.tags, self.relation_dim, self.hidden_dim, 'relation_project')
    neg_tags_p = self._apply_relu(self.neg_tags, self.relation_dim, self.hidden_dim, 'relation_project', True)

    # GRU Layer on p
    z, _ = self._apply_gru(p, self.hidden_dim, 'encoder')
  ...
  def train(self, train_data_producer, valid_data_producer, num_epoch):
    ...
    with tf.Session() as self.sess:
      ...
      for i in range(num_iteration):
        ...
        feed = {self.s: questions, self.tags: embeddings, self.neg_tags: neg_embeddings}
